<template>
	<view class="detail">
		<view class="fixbg" :style="{'background-image':'url('+ Detail.al.picUrl +')'}"></view>
		<musichead :title="Detail.name" :icon="true" color="white"></musichead>
		<view class="container" v-show="!isLoading">
			<scroll-view scroll-y="true" >
				<view class="detail-paly">
					<image :src="Detail.al.picUrl" mode="" :class="{'detail-paly-run' : isPlayRotate}"></image>
			...
			getMusic(songId){
				...
				Promise.all([songDetail(songId) ,songSimi(songId),songComment(songId),songLyric(songId),songUrl(songId)]).then((res)=>{
					if(res[0][1].data.code === 200){
						this.Detail = res[0][1].data.songs[0]
					}
					...
					if(res[4][1].data.code ===200){
						...
						this.bgAudioManager.title =this.Detail.name ;
						// #endif
						// #ifdef H5
...
<style scoped>
	.detail-paly{width: 580rpx; height: 580rpx; background: url(../../static/disc.png); background-size: cover; margin: 214rpx auto 44rpx auto;
	position: relative;}
	.detail-paly image{width: 370rpx; height: 370rpx; border-radius: 50%; position: absolute; left: 0; top: 0; right: 0; bottom: 0;
	 margin: auto; animation: 10s linear move infinite;animation-play-state: paused;}
	 .detail-paly .detail-paly-run{animation-play-state: running;}
	 @keyframes move{
	 	from{transform: rotate(0deg);}
	 	to{transform: rotate(360deg);}
	 }
	 
	.detail-paly text{width: 100rpx;height: 100rpx; font-size: 100rpx; color: white;position: absolute; left: 0; top: 0; right: 0; 
	bottom: 0; margin: auto;}
	.detail-paly view{width: 230rpx; height: 360rpx; background: url(../../static/needle.png);position: absolute; left: 80rpx; 
	top: -200rpx; right: 0; margin: auto; background-size: cover;}
